# csai415-pdf-papers-agent
CSAI415 D1-D4 - PDF-Papers AI Agent
...
├── data/
│   ├── corpus.parquet    ← 150 synthetic arXiv-style papers across 6 topics
...
A single seed (`42`) is used throughout: corpus generation, gold split, Optuna sampler, the incremental classifier, ADWIN.

## Headline results

| Metric | Best baseline (naive 0.5) | AutoML winner (full gold) |
|---|---:|---:|
| Recall@5 | 0.609 | **0.615** |
| NDCG@5  | 0.890 | **0.896** |
| p95 latency | 0.39 ms | 0.48 ms |
...
|---|---:|---:|---:|
| Pre-drift  | 0.94 | 0.81 | (exploration tax) |
| Post-drift | 0.05 | 0.63 | **+1122 %** |
| Overall    | 0.49 | 0.72 | **+46.2 %** |

ADWIN fires at step **639** (~ 39 steps after the injected drift) and resets the classifier, which re-learns under the new regime.

## Notes

- **Dense embedder.** D1 uses TF-IDF + TruncatedSVD instead of `bge-small-en` (sandbox is offline). The `HybridRetriever` interface holds stable so D2 swaps the dense matrix in without touching downstream code.
...
- **Targets met.** Recall@5 ≥ 0.60 ✓ · p95 latency ≤ 2 s ✓ · online lift > +5 % vs static on a temporal slice ✓ (post-drift +1122 %).
...
- **Ahmed — Corpus & gold set.** Procedurally generated the 150-paper synthetic arXiv-style corpus across six topic clusters; built the 78-query gold set (4 broad + 3 targeted + 6 title per topic); implemented `recall_at_k` and `ndcg_at_k` from scratch and the per-retriever `evaluate` helper.
